...
// ai 机器人判断
export const isItARobot = (deptId: number | string) => {
    if (deptId == "1753687739292135425") {
        return true;
    } else {
        return false;
    }
...
export const getDefaultImage = (): string => {
    return "http://117.72.66.176:9001/api/v1/buckets/jilijili-project/objects/download?preview=true&prefix=MjAyNC8wNS8wMS/ogozogonnlLcucG5n&version_id=null";
};
// 获取默认显示图片路径
export const getDefaultShowImage = (): string => {
    return "https://img2.imgtp.com/2024/04/26/WQ3ArIq9.jpg";
};


/**
 * 创建一个音频元素,播放指定地址内容,播放完成后删除音频元素
 * @param url 音频文件的URL
 */
export function playAudio(url: string = 'http://117.72.66.176:9100/jilijili-project/assets/audio/def_01.wav') {
...
// 获取网络头像
export function getNetWorkImage(avatar: string | number) {
    let r = ''
    listByIds(avatar).then(res => {
        const {code, data} = res
        if (code === 200) {
            r = data[0].url
        } else {
...
// 日期个格式化，传入str时间，输出指定格式
export function formatDate(str: string, fmt: string): string {
